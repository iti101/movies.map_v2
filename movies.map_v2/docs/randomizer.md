...
Two bits of state: `phase` is which screen you are on; `status` is how the last roll went.

| `phase` | Screen |
| --- | --- |
| `setup` | Wizard step (`stepIndex` 0–3) |
| `result` | Summary strip + poster / empty / error |
...
**Skip** on Want / Don't want is the same as **Continue** — both call `goNext()` and leave the genre list empty (“any”). Format has no Skip. Changing type / genres / person, or tapping a step pill, sets `phase` back to `setup` and clears the pool.
