...
| Typo suggestion | `src/components/DidYouMean.jsx` (`getSearchSuggestion` in `tmdb.js`) |
...
All of these are `localStorage` keys on this origin. Clearing site data wipes login and lists.

| Key | Written by | What it is |
| --- | --- | --- |
| `theme` | `App.applyTheme` | `'light'` or `'dark'` |
| `noviToken` / `noviUser` | `novi.saveSession` | JWT + `{ id, email, roles, username? }` |
| `isLoggedIn` | `novi.saveSession` / `clearSession` | Leftover `'true'` / `'false'`. `loadSession()` trusts the JWT, not this flag. |
| `watchlists` | `watchlist.js` | `{ [userId]: List[] }` — not synced to Novi |
| `watchRegion` | `tmdb.saveWatchRegion` | 2-letter country for “Where to watch” |
...
| Detail page says “Could not load this page…” | TMDB detail/season fetch failed (`tmdbGet`). Search uses a different error string. |
| “No reviews yet” after you know reviews exist | `GET /api/reviews` failed or Novi returned another title’s rows. Failures look the same as empty. |
